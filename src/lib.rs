...
impl Plugin for Ts3JoinInfo {
    ...
    fn name()        -> String { String::from("Channel Join Info") }
    fn version()     -> String { String::from("0.1.0") }
    fn author()      -> String { String::from("Brakenium") }
    fn description() -> String { String::from("A plugin that will display user join messages in the channel chat.") }
    ...
    fn connection_move(&mut self, api: &mut TsApi, server_id: ServerId, connection_id: ConnectionId, old_channel_id: ChannelId, new_channel_id: ChannelId, _visibility: Visibility) {
        if let Some(server) = api.get_server(server_id) {
            ...
            if Ok(connection_id) != own_connection_id {
                // Get our own connection id
                let own_connection = own_connection_id.ok()
                ...
                match own_channel_id {
                    ...
                    Some(channel_id) if [old_channel_id, new_channel_id].contains(&channel_id) => {
                        ...
                        let mut formatted_message = format!("{} switched from channel {} to {}", connection_mention, old_channel_mention, new_channel_mention);
